...
class GeneralSettingsController extends Controller
{
    ...
    public function update_page_settings(Request $request)
    {
        ...
        $fields = [
            'home_page','pricing_plan','job_page','donation_page','event_page','knowledgebase_page'
        ];
    ...
    {
        $theme_suffix = ['theme_donation', 'theme_job', 'theme_event','theme_support_ticket','theme_ecommerce','theme_knowledgebase'];
        if (tenant()) {
            ...
            $fields = array_merge($fields[0], $fields[1], $fields[2],$save_data[3], $save_data[4], $save_data[5]);
            $this->validate($request,$fields);

            $save_data = array_merge($save_data[0], $save_data[1], $save_data[2],$save_data[3], $save_data[4], $save_data[5]);
            foreach ($save_data as $item) {
                update_static_option($item, $request->$item);
            }

            // Issue to fix
            $font_variant = array_merge($font_variant[0], $font_variant[1], $font_variant[2], $font_variant[3],$font_variant[4],$font_variant[5]);
 ...
 public function update_license_settings(Request $request)
    ...
    private function all_themes_colors_fields() : array
    {
        ...
        return array_merge($donation_color_fields,
            $job_color_fields,
            $event_color_fields,
            $support_ticket_color_fields,
            $ecommerce_color_fields,
