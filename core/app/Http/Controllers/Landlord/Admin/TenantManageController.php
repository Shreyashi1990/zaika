...
class TenantManageController extends Controller
{
    const BASE_PATH = 'landlord.admin.tenant.';
     ...
     public function all_tenants(){
        ...
        $themes = Themes::select('id','title','slug')->get();
    ...
    public function delete($id)
    {
        ...
        foreach ($tenants ?? [] as $tenant)
        {
            ...
            if(file_exists('assets/tenant/frontend/themes/css/dynamic-'.$tenant->id.'-style.css')){
                unlink('assets/tenant/frontend/themes/css/dynamic-'.$tenant->id.'-style.css');
            }

            if(file_exists('assets/tenant/frontend/themes/js/dynamic-'.$tenant->id.'-script.js')){
                unlink('assets/tenant/frontend/themes/js/dynamic-'.$tenant->id.'-script.js');
            }
        }
        ...
        if (!empty($tenant))
        {
            ...
            if (!empty($old_tenant_log->package_id) && !empty($old_tenant_log->user_id) && $old_tenant_log->user_id == $user->id)
            {
                ...
                PaymentLogs::findOrFail($old_tenant_log->id)->update([
                    ...
                    'theme' => $old_tenant_log->theme ?? 'theme-1'
                ]);
